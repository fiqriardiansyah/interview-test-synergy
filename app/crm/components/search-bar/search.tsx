...
export default function Search({ clients, ...props }: Props) {
    const { query, setQuery } = React.useContext(CrmContext);
    ...
    return (
        <div className="relative" {...props}>
            ...
            <Select
                ...
                allowClear
                size="large"
                className="select-clients w-[300px]"
                showSearch
                suffixIcon={null}
                placeholder="Search"
                ...
                {clients?.map((client) => (
                    <Select.Option key={client.id} value={client.name} label={client.name}>
                        <p className="text-base font-medium capitalize">
                            {client.name} <span className="text-gray-400 text-sm font-normal ml-2">@{client.name?.split(" ")[0]}</span>{" "}
                        </p>
                    </Select.Option>
                ))}
            </Select>
